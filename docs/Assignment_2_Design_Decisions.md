...
## Stage 2 - Subtask 11B.5B and XML v2 Production Loading

- Status: Completed. Order Book order submission, matching, initial inventory, DTO details, and UI integration remain deferred to the following milestones.
- Goal: Validate Assignment 2 business rules and connect structural version detection, schema-specific JAXB, immutable definitions, validation, and system construction into one atomic production loading path.
- Validation decision: User and event text is validated after trimming while name uniqueness remains case-sensitive. User initial cash must be greater than zero; event IDs remain unrestricted `int` values and unique; every event must contain exactly two distinct non-blank options; commission must be `0..90`; LMSR `b` and Order Book `d` must be positive; and Order Book `initial` may be zero but not negative.
- Market Maker decision: Every referenced event must exist and every event must be assigned exactly once across all users. Missing references use `EVENT_NOT_FOUND`, missing assignments use `MARKET_MAKER_NOT_ASSIGNED`, and duplicate assignments use `MARKET_MAKER_ALREADY_ASSIGNED`. The instructor `error-2.xml` fails with `INVALID_INITIAL_CASH`; `error-3.xml` fails with `EVENT_NOT_FOUND`.
- Construction decision: Assignment 2 users, ordered events, options, commissions, and MM assignments are created in a fresh `MarketSystem`. All v2 events start `NOT_STARTED` with zero event-account balance. LMSR uses the existing mechanism and calculator. Order Book definitions become immutable configuration-only `OrderBookTradingMechanism` instances; order processing and opening inventory are intentionally deferred to the Order Book milestone.
- Load-result decision: `LoadResult` remains unchanged. Its Assignment 2 `totalInitialSubsidy` is calculated from the same LMSR mechanism instances for LMSR events only; it does not fund event accounts, and Order Book `initial` is excluded.
- Atomic routing decision: `MarketSystemXmlLoader` detects the structural version once and routes only to the matching schema/JAXB/definition pipeline. It returns a complete candidate system and subsidy total. `GuessMarketEngineImpl` replaces `currentSystem` only after every stage succeeds, with no cross-version fallback. The existing Assignment 1 loader, definitions, validator, factory, active-event behavior, and legacy API remain intact.
- Order Book representation decision: `TradingMethod.ORDER_BOOK` and a distinct `OrderBookTradingOperations` configuration contract were added so valid v2 files containing mixed LMSR and Order Book events can be loaded without pretending that Order Book is LMSR. Full Order Book details and mutations are deliberately rejected until their implementation milestones.
- Validation result: The focused v2 mapper, validator, and production-loading suites ran 37 tests with 0 failures and 0 errors. A clean Engine build regenerated JAXB v2 and ran 297 tests with 0 failures and 0 errors. `EngineSmokeTest` passed with assertions enabled. ConsoleUI and JavaFXUI each compiled and ran all 3 tests successfully. Official valid, schema-invalid, business-invalid, hybrid, malformed, DTD, repeated-load, and atomic-preservation paths were covered.
- Commit ID: Recorded in the final run summary after commit creation.
...
## Stage 2 - Order Book Domain Milestone

- Status: Completed. Public Engine DTO/API projection and JavaFX integration remain deferred to the following milestones.
- Goal: Implement the Assignment 2 Order Book lifecycle, matching, minting, accounting, positions, settlement compatibility, and market-state queries without exposing domain objects through `GuessMarketEngine` yet.
- Contract decision: `OrderBookTradingOperations` remains a mechanism-family contract separate from the minimal `TradingMechanism`. It exposes configuration, pure prepare/apply order submission, immutable order and execution snapshots, pending participants, reserved SELL quantity, and `BID`, `ASK`, `LAST`, `MID`, and `SPREAD` queries. LMSR contracts and behavior are unchanged.
- Initial inventory decision: Opening an Order Book event debits the assigned active Market Maker by `initial`, credits the event account by the same amount, and assigns `initial / d` whole pairs, one share in each option per pair. The paid amount is split equally between the two option positions. Because holdings are integral and the specification grants one pair per base value, `initial` must be divisible by `d`; zero remains valid and creates no position.
- Matching decision: Each option has an independent price-time-priority book. A crossing order executes at the resting order's price; full orders are removed and partial orders retain their remaining quantity. The specification does not prohibit matching a user's new order against that user's resting order, so self-orders follow the same deterministic matching and accounting rules rather than introducing an undocumented exception. Cancellation and market orders are not introduced because the assignment does not require them.
- Mint decision: When minting is enabled, opposite-option BUY orders mint equal quantities once their bids sum to at least `d`. The resting order pays its full bid and the incoming order pays the complement to `d`. Both share costs enter the event account, aggregate option supplies increase equally, and each buyer receives the corresponding position.
- Accounting decision: Normal trades transfer share cost directly from buyer to seller and do not change the event account or aggregate supply. ON_PURCHASE commission is charged to the buyer and credited to the Market Maker. When the buyer is the Market Maker, the monetary debit is the share cost only, while the position still records gross commission. SELL execution reduces the seller's shares and proportional paid cost basis while preserving commission history.
- Participation and blocking decision: An unmatched order remains in the mechanism and its owner is reported as a pending participant without creating an empty position. New orders from blocked users are rejected. An active buyer's matching order may complete even when its aggregate debit makes the account negative; that operation remains effective and then marks the buyer `BLOCKED`, preserving the approved account rule.
- Atomicity decision: Order matching first produces an immutable `PreparedOrderSubmission`. `MarketSystem` resolves every actor, consolidates and prevalidates all balance, position, sale, event-account, aggregate-share, commission, and overflow effects before applying the prepared book state and no-fail domain mutations. Reserved pending SELL quantities prevent overselling. A stale plan or any expected domain failure leaves balances, statuses, positions, aggregate shares, event funds, orders, and executions unchanged.
- Settlement decision: The existing Task 2 settlement path applies to Order Book events through `WinningPayoutOperations`, where `d` is the payout per winning share. Initial and mint funds back aggregate issued pairs; normal transfers do not alter liabilities. Closing remains Market-Maker-authorized, validates position aggregates, pays all winners atomically, returns residual event funds to the Market Maker, drains the event account, and changes the event to `CLOSED`.
- Public boundary: `MarketSystem.submitOrder(String, int, int, OrderSide, long, double)` returns immutable domain `OrderSubmissionOutcome`. `GuessMarketEngine`, existing Task 1 and Task 2 DTOs, ConsoleUI, JavaFXUI, XML schemas, and JAXB bindings are unchanged in this milestone.
- Changed files: Order Book mechanism/contract/value types, `MarketSystem`, `MarketEvent`, `MarketPosition`, `UserAccount`, `User`, focused error/order enums, Assignment 2 validation, focused tests, and this decision log.
- Validation result: Focused Order Book, atomic-integration, and Assignment 2 validation suites ran 32 tests with 0 failures and 0 errors. The complete Engine suite ran 320 tests with 0 failures and 0 errors. `EngineSmokeTest` passed with assertions enabled. ConsoleUI and JavaFXUI each compiled and ran all 3 tests successfully. The whitespace and scope audits also passed.
- Commit ID: Recorded in the final run summary after commit creation.

## Stage 10 - Order Book Public API and DTO Milestone

- Status: Completed. JavaFX controller integration remains deferred to the next milestone.
- Goal: Expose the completed Order Book behavior through immutable Task 2 DTOs and `GuessMarketEngine` without exposing domain objects or changing any Assignment 1 signature.
- DTO decision: `OrderBookEventDetails` implements the existing `TradingMechanismDetails` family and carries immutable configuration, per-option books and market statistics, and newest-first execution history. `OrderBookOptionDetails`, `LimitOrderDetails`, and `OrderExecutionDetails` keep orders, fills, `BID`, `ASK`, `LAST`, `MID`, and `SPREAD` free of domain and JavaFX types. `OrderSubmissionResult` returns the submitted-order snapshot, executions from that submission, the updated event, and the updated submitting user.
- API decision: `GuessMarketEngine.submitOrder(int eventId, String userName, int optionNumber, OrderSide side, long quantity, double limitPrice)` is the only new public operation. It delegates to the atomic domain transaction and maps the result without retry or fallback. The seven Assignment 1 signatures and the existing seven Task 2 signatures remain exact.
- Mapping decision: `MarketEventDtoMapper` selects mechanism projections through `TradingMethod` and `OrderBookTradingOperations`, never through a concrete mechanism cast. BUY orders are presented in descending price-time priority and SELL orders in ascending price-time priority. Every returned collection is an immutable snapshot.
- Participation decision: Event and user projections include a user who has either a position or a currently pending Order Book order. A successful unmatched first order therefore appears as a zero-holding event participation without creating a fake domain position. Executed buyers and sellers continue to be represented by their real positions.
- Compatibility decision: Existing LMSR projections, purchase/settlement DTOs, legacy event DTOs, ConsoleUI behavior, XML/JAXB, and domain settlement remain unchanged. The public interface contains no `guessmarket.engine.domain` type and no JavaFX dependency.
